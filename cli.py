...
@optgroup.option("--color-factor", default=1, type=click.IntRange(min=1),
                 ...
                 show_default=True)
# 新添加的
@optgroup.option("--site-color", default="blue", type=click.STRING, show_default=True,
                 help="Color to highlight the sites")
...
def main(**kwargs):
    ...
    for key in kwargs.keys():
        if key in IMAGE_TYPE and kwargs[key] and os.path.exists(kwargs[key]):
            ...
            p.add_sites(kwargs[key], color=kwargs["site_color"])  # 多添加了一个颜色参数
        elif key == "link":
