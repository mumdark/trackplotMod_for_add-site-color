...
class Plot(object):
    ...
    @add_object_error
    def add_sites_old(self, sites, color="blue"):           # 旧的
        u"""
        ...
        if sites is not None:
            ...
            logger.info(f"add sites: {sites} with color: {color}")          # 打印添加的颜色参数
            if isinstance(sites, str):
                ...
                for s in sites:
                    ...
                    if s not in self.sites.keys():
                        self.sites[s] = color          # 添加的参数
                    else:
                        self.sites[s] = "red"
            elif isinstance(sites, int):
                sites = sites - self.start
                if sites not in self.sites.keys():
                    self.sites[sites] = color          # 添加的参数
                else:
                    self.sites[sites] = "red"
        return self

    @add_object_error
    def add_sites(self, sites, color="blue"):
        """
        highlight specific sites
        :param sites: string in 100,200 format or int
        :param color: string in color1,color2 format or single color
        :return:
        """
        assert self.region is not None, "please set plot region first."

        if sites is not None:
            logger.info(f"add sites: {sites} with color: {color}")
            ...
            if isinstance(color, str):
                colors = color.split(",")
            if len(colors) == 1:
                colors = colors * len(sites)

            for i, s in enumerate(sites):
                s = s - self.start
                site_color = colors[i % len(colors)]
                if s not in self.sites.keys():
                    self.sites[s] = site_color
                else:
                    self.sites[s] = "red"
        return self
